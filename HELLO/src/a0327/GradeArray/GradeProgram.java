...
private static void printFile(File file, String[][] student, String dirName) {
   if(checkBeforeWriteFile(file)) {
         ...
         pw.println(" 이름\t나이\t영어\t수학\t총점\t평균");
         for (int i = 0; i < student.length; i++) {
            if(student[i][0] != null)
            pw.println(student[i][0]+"\t"+student[i][1]+"\t"+student[i][2]+"\t"+student[i][3]+"\t"+student[i][4]+"\t"+student[i][5]);
         }
         pw.close();
...
static int addStudent(String[][] student, int dataSize, Scanner sc) {
    ...
    double average = (engScore + mathScore) / 2.0;
    ...
    student[dataSize][4] = String.valueOf(sum);
    student[dataSize][5] = String.valueOf(average);
...
//8-1. 학생 데이터가 있는 배열만 뽑아서 총점과 인덱스를 담는 int 배열만들기
 private static int[][] getSum(String[][] student, int dataSize) {
...
//8-2 학생 성저순으로 저장(출력)하기
 private static int[][] sortByGrade(String[][] student, int dataSize) {
    ...
    double avg = getavgOne(student[index]);
    System.out.println((i+1)+"등 : "+student[index][0] + ", 나이: " +student[index][1] + ", 영어: "+student[index][2] +", 수학: "+student[index][3]+", 총점: "
    + studentGradeAll[i][1]+"점" +", 평균: " +avg);
     } //이름 나이 수학 영어 평균 출력하고 저장까지
    }
